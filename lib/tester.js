...
const [map, forEach, filter] = ['map','forEach','filter'].map(k => [][k].call.bind([][k]))
const save = proto => (entries => () => {
  const changed = map(filter(entries, ([k,v]) => proto[k] !== v), ([k,v]) => [k, proto[k]])

  return {
    init: () => forEach(changed, ([k,v]) => proto[k] = v),
    restore: () => forEach(entries, ([k,v]) => proto[k] = v),
  }
...
const loadCode = async ({ path, api, solution }) => {
  ...
  const [exerciseCode, solutionCode] = await all([
...
const openExercise = async ({ path, api, solution, code }) => {
  ...
  const mod = await api.importCode(code)
    .catch(err => ({ tests: [() => { throw err }] }))

  const ctx = mod.setup ? mod.setup() : {}
  const handlers = map(protoSave, s => s())
  const init = () => forEach(handlers, h => h.init())
  const restore = () => forEach(handlers, h => h.restore())
  restore()
  code = code.split('// /*/ // ⚡')[1]

  return { ...mod, code, init, restore, ctx }
}
...
export const test = async ({ name, timeout, api, failFast, ...rest }) => {
  ...
  for (const t of ex.tests) {
    ...
    const tools = { eq, fail, wait, code: ex.code, ctx: ex.ctx }
    const running = (async () => (ex.init(), t(tools)))()
      .catch(e => (err = e, false))
      .finally(ex.restore)

    if (!await race([ running, pause ])) {
