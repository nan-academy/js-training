...
const openExercise = async (path, api) => {
  const [exerciseCode, solution] = await Promise.all([
    api.readFileOrEmpty(path),
    api.readFileOrEmpty(path.replace(/\bexercise\b/, 'solution')),
  ])
...
const test = async (name, timeout, api) => {
  const ex = await openExercise(`./exercise/${name}.js`, api)
  let i = 0, PASS = true
  const ctx = {}
  const results = []
  for (const t of ex.tests) {
    ++i
    let err = ''
    const tools = { eq: api.eq, fail, wait, code: ex.code }
...
export const run = async (modname, api) => {
  ...
  for (const [ex, props] of Object.entries(exercises)) {
    try { if (!await test(ex, timeout, api)) return }
    catch (err) { return log(err) }
  }
  return next ? run(next, api) : log('All done !')
}
