const importText = (code) =>
  import(URL.createObjectURL(new Blob([code], { type: 'text/javascript' })))

// check import support
const check = Promise.resolve()
  .then(() => importText('export const ok = true'))
  .then(() => importText)
  .catch((err) => (code) => ({
    tests: new Function(`${code.replace(/\nexport /g, '\n')};\nreturn tests`)(),
  }))
...
addEventListener('message', async ({ data: code }) => {
  ...
  try {
    ...
    if (parts.length > 3) throw Error('too many ⚡ separators')
    const { tests, setup } = await exec(code)
    if (!Array.isArray(tests)) {
      throw Error('Missing tests export (invalid module)')
    }
    const ctx = typeof setup === 'function' ? setup() : {}
    _test = async (i) => {
      try {
        const pass = await tests[i]({ eq, code: parts[1], wait, fail, ctx })
    ...
    const error =
      err instanceof Error
        ...
        : `${
            err != null ? err.constructor.name : `Value<${typeof err}>`
          }: ${err}`
